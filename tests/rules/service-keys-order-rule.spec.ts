...
test('ServiceKeysOrderRule: should return a warning when service keys are in the wrong order', (t) => {
  ...
  const correctOrder = [
    'image, build, container_name, depends_on, volumes, volumes_from, configs, secrets, environment, env_file, ports, networks, network_mode, extra_hosts, command, entrypoint, working_dir, healthcheck, restart, logging, labels, profiles, user, isolation, devices, annotations, cpu_rt_period, cpu_rt_runtime',
  ];
